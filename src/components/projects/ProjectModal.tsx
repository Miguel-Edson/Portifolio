  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-isabelline rounded-3xl w-full max-w-4xl max-h-[90vh] overflow-y-auto shadow-2xl flex flex-col md:flex-row animate-in zoom-in-95 duration-300">
        ...
        <button
          onClick={onClose}
          className="absolute top-4 right-4 z-20 p-2 bg-black/50 hover:bg-caribbean/90 rounded-full text-white transition-colors border border-white/20"
        >
          <X size={24} />
        </button>
        ...
        <div className="flex-1 p-6 md:p-12 flex flex-col gap-6 text-caribbean">
            ...
            <p 
                className="text-lg leading-relaxed text-gray-700 line-clamp-6" 
                title={project.fullDescription} 
            >
            ...
            <div>
                ...
                {project.skills.map((skill) => (
                    <span
                    key={skill}
                    className="px-4 py-1.5 rounded-full bg-caribbean/10 text-caribbean font-medium text-sm border border-caribbean/90"
                    >
                    {skill}
                    </span>
                ))}
